...
class BaselineCorrection(ToolBase):
    ...
    def baseline(spc, config, progress_callback):
        ...
        for n, di in enumerate(spc.data):
            ...
            if algorithm == 'median':
                ...
                dr = ng.process.proc_bl.cbf_explicit(di, calc=slice(cbf_explicit_start, cbf_explicit_end))
        ...
        return {'spc': spc, 'baseline': np.array(bls), 'baseline_point_idx': idx, 'baseline_point_y': np.mean(np.array(bls_points), axis=0)}
    ...
    def plot(self, **kwargs):
        ...
        if 'baseline_point_idx' in self.data and self.data['baseline_point_idx'] is not None:
            idx = self.data['baseline_point_idx']
            canvas = self.parent().spectraViewer.spectraViewer

            mean_spc = np.mean(self.data['spc'].data, axis=0)
            canvas.plot(self.data['spc'].ppm[idx], self.data['baseline_point_y'], pen=None, symbol='o', symbolBrush=QBrush(QColor('blue')))
